...
knn_df = pd.read_csv("credit_data.csv",names=["SlNo","Age","Sex","Job","Housing","Saving accounts","Checking account","Credit amount","Duration","Purpose","Risk"],header=0,index_col = 0)
...
knn_df['Risk'] = knn_df['Risk'].apply(lambda x: 0 if x == "bad" else 1)
print("\n",knn_df)

# Use the Standard Scaler on the criteria
#s_scaler = StandardScaler()
knn_input = knn_df.drop('Risk',axis=1)
...
X_train, X_test, y_train, y_test = train_test_split(knn_input,knn_df['Risk'], test_size=0.33)
